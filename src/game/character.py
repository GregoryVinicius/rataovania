...
class Character(Entity):
    ...
    def apply_gravity(self, gravity=None, time=1):
        if not self.on_ground:
            # Atualiza a velocidade vertical considerando a gravidade
            self.y_velocity += gravity * time
            # Atualiza a posição do personagem
            self.rect.y += int(self.y_velocity * time)
    ...
    def jump(self, jump_force=-15):
        if self.on_ground:
            self.y_velocity = jump_force  # força negativa para subir
